...
assert(radSw.includes("exopace-radio-v47"), "Radio SW cache bumped");
...
assert(radioHtml.includes("app.js?v=47") && radioHtml.includes("env.js?v=47"), "Radio index cache-busts in-place JS");
...
assert(index.includes('showHudToast("NO MATCH")') && index.includes("findBody") && index.includes("parseTleBodies"), "search no-match toasts NO MATCH instead of bundle NO LOCK");
assert(index.includes("clearSearchInput") && index.includes('form.classList.contains("search")'), "failed search clears the query field");
assert(index.includes("function stripRejectedShare") && index.includes('q.delete("lock")') && index.includes('q.delete("cam")'), "invalid share lock strips rejected lock/cam so the URL matches NO LOCK paint");
assert(index.includes("function watchDeadShare") && index.includes("shareQuery().get(\"lock\")"), "dead share links wait for catalog then toast NO MATCH");
assert(index.includes('showHudToast("SATELLITES ON")') && index.includes("function ensureSatsForLock") && index.includes('layerBtn("SATELLITES")'), "valid lock auto-enables SATELLITES and toasts SATELLITES ON only when the layer was off");
assert(index.includes("ensuredLockKey === key") && index.includes('btn.classList.contains("on")'), "already-on SATELLITES does not get a second toast");
assert(!index.includes('q.set("layers"') && !index.includes('q.set("sats"'), "share URL does not grow a layer-preference param");
assert(index.includes('showHudToast("NO LOCK")') && index.includes('t === "FLY"'), "FLY / SAT-CAM / FOLLOW still refuse with NO LOCK");
