...
assert(mocSw.includes("exopace-moc-v26"), "MOC SW cache bumped");
...
assert(radSw.includes("exopace-radio-v33"), "Radio SW cache bumped");
...
assert(read("index.html").includes("moc-phone.css?v=26"), "index cache-busts moc-phone.css");
assert(read("index.html").includes("index-B5yAHF7-.js?v=26"), "index cache-busts hashed MOC bundle");
...
assert(radioHtml.includes("app.js?v=33") && radioHtml.includes("env.js?v=33"), "Radio index cache-busts in-place JS");
...
assert(moc.includes("NEXT · AOS ") && moc.includes("Z AZ ") && moc.includes('"NO PASS"'), "lock next-pass paints AOS/AZ when a pass exists, else NO PASS");
assert(!moc.includes("NEXT PASS · EXOPACE GS"), "lock does not paint NEXT PASS heading without AOS/AZ");
...
assert(read("moc-phone.css").includes("#globe .cesium-viewer-toolbar") && read("moc-phone.css").includes("min-height: 44px"), "phone Cesium toolbar and RADIO taps are ≥44");
