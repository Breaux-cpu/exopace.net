...
assert(moc.includes('if(!L){x.setSelected(null),Z({selected:null});const u=x.rig.mode;u==="follow"||u==="satcam"?(x.setMode("moc"),x.recageHome(),Z({cam:"moc"}),bo(null,"moc")):bo(null,u||"moc");return}'), "empty-canvas miss drops only lock and keeps truthful cam/t");
assert(!/if\(!L\)\{x\.setSelected\(null\),Z\(\{selected:null\}\);return\}/.test(moc), "empty-canvas miss no longer leaves lock= in the share URL");
assert(moc.includes("onPick:L=>{if(!L){") && !moc.includes('if(!L){x.setSelected(null),Z({selected:null});return}'), "globe miss uses the CLEAR LOCK share writer without inventing a toast");
