...
assert(mocSw.includes("exopace-moc-v63"), "MOC SW cache bumped");
...
assert(read("index.html").includes("moc-phone.css?v=63"), "index cache-busts moc-phone.css");
assert(read("index.html").includes("index-B5yAHF7-.js?v=63"), "index cache-busts hashed MOC bundle");
...
assert(moc.includes('if(!a.selected()){Ie("NO LOCK");return}a.rig.startCinematic()'), "CINE without a lock toasts NO LOCK and does not start");
...
assert(index.includes('id="exo-toast"') && index.includes('showHudToast("NO LOCK")') && index.includes('SAT-CAM') && index.includes('t === "FLY"') && index.includes('t === "FOLLOW"') && index.includes('t === "CINE"'), "SAT-CAM / FLY / FOLLOW / CINE without a lock share the same NO LOCK refusal");
