...
assert(index.includes("requestAnimationFrame(tickStationClock)") && index.includes("applyStationClock"), "STATION LOCAL stays pinned via rAF under React re-renders");
assert(index.includes('hourCycle: "h23"') && index.includes("patching"), "STATION LOCAL uses America/Chicago h23 and skips observer echo");
...
assert(index.includes("__exoLockBoot") && index.includes("queryLockUrl") && index.includes("rewriteLockUrl"), "lock deep links use ?lock= query state not /lock/ paths");
assert(index.includes("syncLockUrl") && index.includes("applyRateT"), "lock URL keeps cam and LIVE/HOLD/10×/60× in sync via replaceState");
assert(index.includes('showHudToast("NO MATCH")') && index.includes("invalidRefused"), "invalid ?lock= NORAD boots safely with NO MATCH");
